...
`LOYVERSE_ACCESS_TOKEN` is required before real Loyverse order sync can be tested. `LOYVERSE_STORE_ID` and `LOYVERSE_PAYMENT_TYPE_ID` are optional; the API will use the first store and first cash payment type when they are not set.
